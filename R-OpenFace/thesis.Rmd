...
(highlyCorrelated <- findCorrelation(correlation, cutoff=0.75, names = T, exact = T))
```
...
model1 <- train(truthful ~ . , data = ALL_train, method = "knn",
...
# G = 0.03, C = 1 --> 0.5673334
svm.model_lin_all <- parallelSVM(truthful ~ . , data = ALL_train, numberCores = detectCores(), type = "C-classification", kernel = "linear", cost = 1, gamma = 0.04, scale = FALSE)
