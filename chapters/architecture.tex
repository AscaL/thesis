...
\begin{itemize}
	\item Gather an video of a person either lying or telling the truth.
	\item Detect the face of the person in the video.
	\item Detect the facial landmarks.
	\item Ex
\end{itemize}
...
Generally the procedure is as follows (\ref{fig:CLM}):
\begin{itemize}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{DISFA_AU}
...
This allows to detect the following AUs (\ref{fig:all_AUs}):
...
\begin{itemize}[noitemsep]
	...
	CK+ also provides baseline results for facial feature tracking and action unit and emotion recognition.
...
After a lot of testing with different techniques we decided to perform deception detection using an SVM, since it performed the best compared to other methods. 
...
Being in an acceptable condition means that the face should be clearly visible, and the video has to have a the specific person visible and talking in \textit{most} of the video, and no other people on it, so to avoid training on different subjects of which we are not sure if they are lying or telling the truth.

As a consequence many videos had to be cut or discarded to reach an acceptable level.

After extracting Action Units intensity and presence from our video file dataset (\ref{rldb}) we put the data in a csv format to be able to pass them to the SVM. 

The data are divided for both AUs intensity, ranging from $0$ to $1$, and AU presence, having the boolean value of either $0$ or $1$ if the AU was missing or present in the specific frame of the video.
