...
The cost $c$ and $\gamma$ parameter were found using a tuning function that performs a grid search over the combinations of cost and gamma for both kind of kernels, and then by empirically changing the values to see if better results could be achieved. $\gamma$ varied from $10^{-5}$ to $10^{-1}$ and \textit{c} from $10^{-3}$ to $10^2$. After the grid search I did some experimental changes and found that the best parameters were $C = 1$ and $\gamma = 0.04$
